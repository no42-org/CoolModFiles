...
describe("resolveSafe", () => {
  ...
  it("resolves a nested path", async () => {
    ...
    expect(real).toBe(await fs.realpath(path.join(root, "sub", "nested.mod")));
  });
  ...
  it("throws ENOENT for a non-existent path inside root", async () => {
    await expect(resolveSafe("does-not-exist.mod", root)).rejects.toMatchObject(
      { code: "ENOENT" }
    );
  });

  it("rejects a symlink that resolves outside root", async () => {
    await expect(resolveSafe("symlink-to-outside", root)).rejects.toMatchObject(
      { code: "EACCES" }
    );
  });
