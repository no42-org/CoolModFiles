...
export default function SpectrumStyleLed({
  ...
  React.useEffect(() => {
    ...
    const draw = () => {
      ...
      const drawGlowingChromeAndTiles = (pass: number) => {
      ...
      // Tiles — lit only; no off-state. Glow still active from the
      // chrome-text block above; just swap the fill colour.
      ctx.fillStyle = TILE_FILL;
      for (let i = 0; i < NUM_BANDS; i++) {
        const amp = bars[i] / 255;
        const litRows = Math.round(amp * tileRows);
        const xLeft =
          barAreaLeft + i * (bandWidth + BAND_GAP_PX);
      ...
      for (let pass = 0; pass < GLOW_PASSES; pass++) {
        drawGlowingChromeAndTiles(pass);
      }
