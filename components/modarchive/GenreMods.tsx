...
function GenreMods({ id, onPick }: GenreModsProps) {
  ...
  React.useEffect(() => {
    ...
    const url =
      page === 1
        ? `/api/modarchive/genre/${id}`
        : `/api/modarchive/genre/${id}?page=${page}`;
    fetch(url)
      .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
      .then((data: GenreModsResponse) => {
        if (cancelled) return;
        setState({
          status: "ok",
          items: data.items,
  ...
  if (state.items.length === 0) {
    return <div className={styles.empty}>No mods in this genre.</div>;
  }
